...
int getCursorPosition(int ifd, int ofd,unsigned int *rows, unsigned int *cols) {
    char buf[32];
    unsigned int i = 0;
...
void editorInsertRow(unsigned int at, const char *s, unsigned int len) {
    ...
    if (at != E.numrows) {
        ...
        for (unsigned int j = at+1; j <= E.numrows; j++) E.row[j].idx++;
    }
    E.row[at].size = len;
...
void editorDelRow(unsigned int at) {
    ...
    for (unsigned int j = at; j < E.numrows-1; j++) E.row[j].idx++;
    E.numrows--;
    E.dirty++;
}
...
int editorSave(void) {
    ...
    if ((unsigned int)write(fd,buf,len) != len) goto writeerr;
...
void editorRefreshScreen(void) {
    ...
    int err = snprintf(status, sizeof(status), "%.20s - %d lines %s",
        E.filename, E.numrows, E.dirty ? "(modified)" : "");
	if(err == -1){
		exit(1);
	}
	unsigned int len = (unsigned int )err;
    err= snprintf(rstatus, sizeof(rstatus),
